...
var showAnswerer = function(answerer) {
	...
	answererElem.html('<p>Name: <a target="_blank" '+
		...
		'</a><br><img src="' + answerer.user.profile_image + '" alt = "user image"></p>');
	var ansReputation = result.find('.reputation');
	ansReputation.text(answerer.user.reputation);

	var ansPostCount = result.find('.post-count');
	ansPostCount.text(answerer.post_count);

	var ansScore = result.find('.score');
...
var getAnswerers = function(answerers) {
	...
	.done(function(result){ //this waits for the ajax to return with a succesful promise object
		...
		//The function is executed once for each item in the array.
		$.each(result.items, function(i, item) {
			var myAnswerer= showAnswerer(item);
			$('.results').append(myAnswerer);
		});
	})
	.fail(function(jqXHR, error){ //this waits for the ajax to return with an error promise object
		var errorElem = showError(error);
